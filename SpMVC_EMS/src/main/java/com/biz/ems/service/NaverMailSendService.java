...
public class NaverMailSendService {
	...
	public void sendMail(EmsVO emsVO) {
		...
		/*
		 * Mime Type 인터넷 TCP/IP 를 통해서 주고받는 문서(파일)등은 고유의 형식을 갖는다. 그러한 형식을 마임타입이라고 한다.
		 * Hyper Text Markup tag로 만들어진 문서를 HTML DOC Type이라고 하며 이 문서는 Web Browser를 통해서
		 * 내용을 확인할 수 있다. 이처럼 Mime Type을 지정해둔 문서는 인터넷으로부터 받았을 때 즉시 내용을 확인할 수 있다. xMail을
		 * 통해서 지금 만드는 Mime Type는 email viewer에서 확인할 수 있는 문서 형식이다. 이 문서는 메모장이나, Web
		 * Browser로 보면 그 내용을 정확히 확인하기 어렵다. 대표적으로 xMail Mime type문서를 볼 수 있는 어플리케이션에 아웃룩이
		 * 있고, 각 스마트기기의 고유 Email App이 여기에 해당한다.
		 */
		...
		try {
			// 두번째 값을 true : HTML 적용을 한 메시지를 보낼 수 있다.

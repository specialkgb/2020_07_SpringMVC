...
public class ProductController {
	...
	public String proHome(Model model, HttpSession httpSession) {
		...
		// 로그인이 필요한 페이지를 접근할 때 아래 처럼 하면 된다.
	 ...
	 * "query_이름"과 파라메터 변수이름이 같을 때는 생략해도 된다.
	 * 단, 이 Annotation은 primitive 형 이거나 wrapper class(Integer, Character, String)일 경우만
	 * 사용가능하다.
	 * 
	...
	public String delete(@RequestParam("id") String p_code) {
		 ...
		 * 
		 * 다시한번 /product/ URL로 request를 수행해달라는 요청
		 * 상태코드 304번
		 */
		return "redirect:/product/";
	}

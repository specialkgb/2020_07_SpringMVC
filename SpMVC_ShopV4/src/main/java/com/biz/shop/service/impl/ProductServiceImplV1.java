...
public class ProductServiceImplV1 implements ProductService {
	...
	public int update(ProductVO vo) {
		...
		// Update를 수행하기 전에 삭제 flag 칼럼을 강제로 null로 설정하여
		// 해당 상품이 삭제표시가 되지 않도록 하여 놓는다.
	...
	/*
	 * 상품정보, 거래처정보, 회원정보와 같은 데이터를 통상 Master Data라고 한다.
	 * Master Data는
	 * 	WorkData(매입매출, 거래처 외상정보, 회원로그인 Log와 같은)의
	 * 			기준이 되는 값을 가지는 Data Table이다.
	 * 
	 *  어플을 사용해서 어떤 업무를 진행하는 과정에서
	 *  		WorkData에 저장되는 데이터는
	 *  		필수적으로 Master Data와 Join 관계가 맺어진다.
	 *  
	 *  만약 WorkData와 Master Data를 Join하여 어떤 통계정보를 찾고자 할 때
	 *  		Master에 해당하는 데이터(코드와 일치하는)가 없으면
	 *  		WorkData의 정보가 어떤 내용인지 확인하기 어려워지는 경우가 많다.
	 *  
	 *  통상적으로
	 *  Master Data는 원칙상 PK 칼럼 값은 변경하지 않고
	 *  또한 한번 Insert된 Master Data는 삭제하지 않는다.
	 *  필요가 없어진(앞으로 사용하지 않을) 데이터는
	 *  table에서 delete를 하지 않고 특정한 칼럼을 하나 지정하여
	 *  그 칼럼에 flag를 세팅하고 삭제되었음을 표시한다.
	 *  
	 *  1. id(p_code)로 table에서 데이터를 조회하여 VO에 담고
	 *  2. VO의 p_not_use 칼럼을 null이 아닌 값으로 세팅하여
	 *  3. update를 수행한다.
	 *  4. SELECT를 수행할 때 p_not_use 칼럼이 NULL 데이터만 조회하여

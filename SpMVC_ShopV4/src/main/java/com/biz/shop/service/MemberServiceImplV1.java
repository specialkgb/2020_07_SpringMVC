...
public class MemberServiceImplV1 implements MemberService {
	 ...
	 * 
	 * 2. 2번째 부터 가입된 회원은 권한(ROLE)을 "USER"로 부여한다.
	 * 
	 * 3. 회원이 탈퇴를 요청하면 권한(ROLE)을 "GUEST"로 변경한다.
	 * 
	 * 4. 같은 아이디로 재가입을 요청하면 권한을 "GUEST"에서 "USER"로 변경한다.
	 * 
	 * 5. "ADMIN" 권한은 현재 App의 모든 기능을 사용할 수 있다.
	 * 		- 다른 사용자의 정보를 변경할 수 있다.
	 * 		- 다른 사용자의 권한(ROLE)을 변경할 수 있다.
	 * 
	 ...
	 * 
	 * * 회원 가입 절차
	 * 1. tbl_member table에 회원정보가 1개라도 있는지 검사
	 * 2. 없으면 : 현재 추가할 회원은 "ADMIN"
	 * 3. 있으면 : 현재 추가할 회원은 "USER"
	 * 4. memberVO의 ROLE 칼럼에 권한 문자열을 추가하고
	 * 5. INSERT를 수행
	 */
	@Override
	public int insert(MemberVO memVO) {
		 ...
		 * 하지만, 실제 상황에서 JDBC를 통해서 서버에 원격연결을 하는 상황에서는
		 * Count값이 실제로는 0이 아닌 -1 등의 값이 될 수도 있다.
		 * 데이터가 없을 때, 0이거나 -1인 경우가 된다면
		 * if(count == 0 || count == -1) 또는
		 * if(count <= 0) 과 같은 비교연산식이 만들어져야 한다.
		 * 
		 * 비교 연산식에서 가장 권장하는(좋은) 코드는 가급적 비교대상을 부등호로 한번만 비교하는 것.

...
import com.biz.read.model.Book_JSON_Parent;
import com.biz.read.model.Book_XML_Parent;
...
public class NaverServiceImplV2_XML extends NaverServiceImplV1 {
	...
	public List<BookVO> getNaverList(String queryURL) {
		...
		// 많은 기능들이 추가되어 있다.
		// RestTemplate를 사용하기 위해서는
		// queryURL 문자열을 URL 객체가 아닌 URI 객체로 만들어야 한다.
		URI restURI = null;
		/*
		 * springframework에서 외부 API를 사용하여 데이터를 가져올 때
		 * 기존에는 JSON(XML) 형식으로 가져오고, 여러가지 외부 라이브러리를 사용하여 객체로 parsing하는 과정을
		 * 복잡하게 만들어야 했다.
		 * 
		 * spring에서 외부 API를 사용하여 데이터를 가져올 일이 점점 많아지면서
		 * RestTemplate라는 클래스를 새로 만들어 framework프로젝트에서 사용할 수 있도록 만들어 두었다.
		 * 
		 * HttpHeader, HttpEntity, ResponseEntity 객체만 잘 작성하면
		 * 외부 API에 요청하고, 응답받은 데이터를 parsing하는 절차를 대신 수행해준다.
		 */

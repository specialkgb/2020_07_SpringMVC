...
public class BlogFileService {
	...
	/*
	 * ServletContext tomcat 위에서 실행되는 WA(Web App)의 모든 정보를 담고 있는 객체 이미 Spring
	 * project에서는 ServletContext 클래스의 객체 인스턴스가 만들어 있기 때문에 가져다 사용하기 위해서 Autowired로
	 * 묶어주기
	 */
...
//		this.serverRootPath = context.getRealPath("/");
	...
	public List<BlogVO> selectAll() {
		...
		try {
			...
			// 파일 관련 연산을 수행할 때 사용할 file 객체 생성
	...
	/*
	 * blog 글쓰기를 수행하고 저장을 하면 server root Path에 blog.txt라는 파일로 저장하겠다.
	 */
	...
	public void insert(BlogVO blogVO) {
		...
		try {
			...
			String strBlog = String.format("%s:%s:%s", blogVO.getBl_title(), blogVO.getBl_contents(), blogVO.getBl_user());
